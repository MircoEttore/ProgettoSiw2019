...
<body style="background-color: #092b10; color: white;">
	<c:if test="${username != null}">
						...
						<div class="pull-left">
							<ul class="nav navbar-nav">
								...
								<li><button type="button"
										onclick="location.href='logout?logout=true' "
										class="btn navbar-btn btn-danger" value="Log Out">Log
										Out</button></li>
		...
		<div>
			<div class="col-sm-2"
				style="position: fixed; background-color: #2d3542;">
			...
			<div class="col-sm-10" style="padding-left: 350px">
				...
				<div>
						...
						<%
							System.out.print("-1-1-1-1-");
								...
								if (request.getSession().getAttribute("username") != null
									...
									int idCarrello = DatabaseManager.getInstance().getDaoFactory().getUtenteDAO()
											.CercaIdCarrello(utente.getIdUtente());
									System.out.print(idCarrello);
									List<Canzone> lista = new ArrayList<>();
									lista = DatabaseManager.getInstance().getDaoFactory().getUtenteDAO()
											.CercaCanzoniCarrello(idCarrello);
									System.out.println("La size è :" + lista.size());

									for (Canzone song : lista) {
										out.println("<li class=\"row\"><form><span class=\"itemName\">"
												+ song.getArtista().getNomeArtista() + "  " + song.getTitolo() + "</span></form>"
					...
					<div>
						<button class="btn btn-outline-success my-20 my-sm-10"
							onclick="location.href='Cart.jsp'">CheckOut</button>
					</div>
	...
	<c:if test="${username == null}">
		<meta http-equiv="Refresh"
			content="0; http://localhost:8080/IngSWMicroK/index.html" />
	</c:if>
</body>
